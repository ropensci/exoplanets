...
[![Actions Status](https://github.com/tylurp/exoplanets/workflows/build/badge.svg)](https://github.com/tylurp/exoplanets/actions)
...
By default, exoplanets uses `httr` to display the progress of each request and `readr` to display the column specification:
...
This is a basic example which shows you how to access data from the exoplanet table. Note that all these examples have `progress` and `col_spec` set to FALSE, the default is TRUE. I'm setting these parameters to FALSE to avoid a really long README file:
...
To summarise the database, you can use `exo_summary`, which doesn't include everything found [here](https://exoplanetarchive.ipac.caltech.edu/docs/counts_detail.html) (yet) but might in the future. Notice the default output is a list, you can override this with `output = "dataframe"`:
